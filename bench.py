...
def create_global_array(global_shape=(32, )):
    ...
    gspmd_array = jax.make_array_from_callback(
        global_shape, sharding,
        lambda _: jnp.ones(local_shape, dtype=jnp.float32) * jax.process_index(
        ))
...
def all_reduce_nccl(operand):
    return nmb.ops.collective_call(operand, nmb.Backend.NCCL,
                                   nmb.Collective.AllReduce,
                                   nmb.Mode.OutOfPlace)
...
def all_reduce_mpi(operand):
    return nmb.ops.collective_call(operand, nmb.Backend.MPI,
                                   nmb.Collective.AllReduce,
                                   nmb.Mode.OutOfPlace)
...
@partial(shard_map,
         ...
         out_specs=P('gpus'),
         check_rep=False)
def asym_reduce_nccl(operand):
    return nmb.ops.collective_call(operand, nmb.Backend.NCCL,
                                   nmb.Collective.AsymReduce,
                                   nmb.Mode.OutOfPlace)
...
for size_in_gb in sizes_in_gb:
    ...
    result_asym_reduce_nccl = asym_reduce_nccl(gspmd_array)
    del result_asym_reduce_nccl
