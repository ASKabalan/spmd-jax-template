...
enum class Collective { AllReduce, AllGather, AllToAll, AsymReduce, Peer2Peer };

std::string collective_str(const Collective &c) {
    switch (c) {
        case Collective::AllReduce:
            return "AllReduce";
        case Collective::AsymReduce:
            return "AsymReduce";
...
ffi::Error CollectiveImpl(cudaStream_t stream, int64_t backend, int64_t collective, int64_t mode,
                          ffi::Buffer<ffi::DataType::F32> x, ffi::Result<ffi::Buffer<ffi::DataType::F32>> y) {
    ...
    // size;
    // split the comms to two splits
    ncclComm_t split_comm;
    ncclCommSplit(comm, rank / 2, rank, &split_comm, nullptr);
    // split MPI comm
    MPI_Comm split_mpi_comm;
    MPI_Comm_split(MPI_COMM_WORLD, rank / 2, rank, &split_mpi_comm);
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        if (e_backend == Backend::MPI) {
            switch (e_collective) {
                case Collective::AllReduce:
                    if (e_mode == Mode::InPlace) {
                        MPI_Allreduce(MPI_IN_PLACE, y->untyped_data(), y->element_count(), MPI_FLOAT, MPI_SUM,
                                      MPI_COMM_WORLD);
                    } else {
                        MPI_Allreduce(x.untyped_data(), y->untyped_data(), y->element_count(), MPI_FLOAT,
                                      MPI_SUM, MPI_COMM_WORLD);
                    }
                    break;
                case Collective::AsymReduce:
                    MPI_Reduce(MPI_IN_PLACE, y->untyped_data(), y->element_count(), MPI_FLOAT, MPI_SUM, 0,
                               split_mpi_comm);
                    MPI_Reduce(MPI_IN_PLACE, y->untyped_data(), y->element_count(), MPI_FLOAT, MPI_SUM, 1,
                               split_mpi_comm);
                    break;
                case Collective::AllGather:
                    MPI_Allgather(x.untyped_data(), x.element_count(), MPI_FLOAT, y->untyped_data(),
                                  x.element_count(), MPI_FLOAT, MPI_COMM_WORLD);
                    break;
                case Collective::AllToAll:
                    if (e_mode == Mode::InPlace) {
                        MPI_Alltoall(MPI_IN_PLACE, 0, MPI_FLOAT, x.untyped_data(), x.element_count(),
                ...
                case Collective::Peer2Peer:
                    if (rank < next_rank) {
                        MPI_Send(x.untyped_data(), x.element_count(), MPI_FLOAT, next_rank, 0,
                                 MPI_COMM_WORLD);
                        MPI_Recv(y->untyped_data(), y->element_count(), MPI_FLOAT, next_rank, 0,
            ...
            switch (e_collective) {
                case Collective::AllReduce:
                    ncclAllReduce(x.untyped_data(), y->untyped_data(), x.element_count(), ncclFloat32,
                                  ncclSum, comm, stream);
                    break;
                case Collective::AsymReduce:

                    ncclReduce(x.untyped_data(), y->untyped_data(), x.element_count(), ncclFloat32, ncclSum,
                               0, split_comm, stream);
                    ncclReduce(y->untyped_data(), y->untyped_data(), x.element_count(), ncclFloat32, ncclSum,
                               1, split_comm, stream);

                    break;
                case Collective::AllGather:
                    ncclAllGather(x.untyped_data(), y->untyped_data(), x.element_count(), ncclFloat32, comm,
                                  stream);
                    break;
                case Collective::AllToAll:
                    ncclGroupStart();
                    for (int r = 0; r < size; r++) {
                        ncclSend((void *)(x.typed_data() + r * chunk_size), chunk_size, ncclFloat32, r, comm,
                                 stream);
                        ncclRecv((void *)(y->typed_data() + r * chunk_size), chunk_size, ncclFloat32, r, comm,
                                 stream);
                    }
                    ncclGroupEnd();
                    break;
                case Collective::Peer2Peer:
                    ncclGroupStart();
                    ncclSend(x.untyped_data(), x.element_count(), ncclFloat32, next_rank, comm, stream);
                    ncclRecv(y->untyped_data(), y->element_count(), ncclFloat32, next_rank, comm, stream);
                    ncclGroupEnd();
                    break;
            }
        }
        ...
        if (enable_perf) {
            MPI_Allreduce(MPI_IN_PLACE, &elapsed_ms, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
            if (rank == 0) {
                elapsed_ms /= size;
                double elapsed_s = elapsed_ms / 1E3;
                double alg_bandwidth = (double)(x.size_bytes() + y->size_bytes()) / 1E9 / elapsed_s;
                double bus_bandwidth;
                std::cout << "Elapsed: " << elapsed_ms << " ms" << std::endl;
                std::cout << "size in bytes: " << x.size_bytes() + y->size_bytes() << std::endl;
                ...
                std::string data_size = doubleToString((x.size_bytes() + y->size_bytes()) / 1E9) + " GB";
    ...
    ncclCommDestroy(split_comm);
    MPI_Comm_free(&split_mpi_comm);
...
NB_MODULE(gpu_ops, m) {
    ...
    nb::enum_<Backend>(m, "Backend").value("NCCL", Backend::NCCL).value("MPI", Backend::MPI).export_values();
    ...
    nb::enum_<Collective>(m, "Collective")
            ...
            .value("AsymReduce", Collective::AsymReduce)
